...
- For job details, mention pay, the rough monthly rupiah figure from payIdrMonthly if present (say it as "about N million rupiah a month"), and eligibility: "id_friendly" means the employer says Indonesia is welcome; "unknown" means the listing does not say, so the user should check; "restricted" means it is region-locked. Then tell them the Apply button is on their screen. Never read the link.
